...
interface Filters {
  horizontalConfiguration: string[]
  verticalConfiguration: string[]
  windDirection: string[]
  areaDensity: string[]
  fileType: string[] // Added fileType filter
}
...
export default function DataTable() {
  ...
  const [filters, setFilters] = useState<Filters>({
    ...
    windDirection: [],
    areaDensity: [],
    fileType: [], // Initialize fileType filter
  })
  ...
  const [openFilters, setOpenFilters] = useState<{ [key: string]: boolean }>({
    ...
    windDirection: false,
    areaDensity: false,
    fileType: false, // Initialize fileType filter UI state
  })
    ...
    const sortedData = data.filter((item) => {
      ...
      const folderMatches =
        (filters.horizontalConfiguration.length === 0 ||
          filters.horizontalConfiguration.includes(item["Horizontal Configuration"])) &&
  ...
  const uniqueValues = useMemo(() => {
    ...
    return {
      horizontalConfiguration: sortStrings(Array.from(new Set(data.map((item) => item["Horizontal Configuration"])))),
      ...
      fileType: fileTypes, // Add file types
    }
  }, [data])
                      ...
                      content={
                        <div className="max-w-xs">
                          {key === "horizontalConfiguration" ? (
                            ...
                            <p className="text-gray-500 size-sm">
                              Filter by file type extension. Shows folders containing at least one file of the selected
                              type.
                            </p>
                          ) : (
                      ...
                      {uniqueValues[key as keyof typeof uniqueValues].map((value) => (
                        <div key={value} className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={() => handleFilterChange(key as keyof Filters, value?.toString() || '')}
                          />
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-32">
                      <Button variant="ghost" onClick={() => handleSort("Plan Area Density")}>
                        Plan <br />
                        Area Density
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </Button>
